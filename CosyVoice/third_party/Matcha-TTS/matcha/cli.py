...
MULTISPEAKER_MODEL = {"matcha_vctk": {"vocoder": "hifigan_univ_v1", "speaking_rate": 0.85, "spk": 0, "spk_range": (0, 107)}}
...
def load_vocoder(vocoder_name, checkpoint_path, device):
    ...
    if vocoder_name in ("hifigan_T2_v1", "hifigan_univ_v1"):
        ...
        raise NotImplementedError(f"Vocoder {vocoder_name} not implemented! define a load_<<vocoder_name>> method for it")
...
def validate_args_for_multispeaker_model(args):
    ...
    if args.spk is not None:
        assert args.spk >= spk_range[0] and args.spk <= spk_range[-1], f"Speaker ID must be between {spk_range} for this model."
    else:
...
def cli():
    ...
    parser.add_argument("--batch_size", type=int, default=32, help="Batch size only useful when --batched (default: 32)")
