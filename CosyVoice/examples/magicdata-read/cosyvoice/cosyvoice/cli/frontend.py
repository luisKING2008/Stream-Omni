...
class CosyVoiceFrontEnd:
    def __init__(self, get_tokenizer: Callable, feat_extractor: Callable, campplus_model: str, speech_tokenizer_model: str, spk2info: str = "", instruct: bool = False, allowed_special: str = "all"):
    ...
    def _extract_speech_token(self, speech):
        ...
        speech_token = (
            self.speech_tokenizer_session.run(None, {self.speech_tokenizer_session.get_inputs()[0].name: feat.detach().cpu().numpy(), self.speech_tokenizer_session.get_inputs()[1].name: np.array([feat.shape[2]], dtype=np.int32)})[0]
            .flatten()
            .tolist()
        )
    ...
    def get_spk_embedding(self, spk_id):
        embedding = self.spk2info[spk_id]["embedding"]
        return embedding
