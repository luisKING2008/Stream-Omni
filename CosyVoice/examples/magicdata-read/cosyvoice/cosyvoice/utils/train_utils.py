...
def init_dataset_and_dataloader(args, configs, gan):
    ...
    cv_data_loader = DataLoader(cv_dataset, batch_size=None, pin_memory=args.pin_memory, num_workers=args.num_workers, prefetch_factor=args.prefetch)
...
def wrap_cuda_model(args, model):
    ...
    if args.train_engine == "torch_ddp":  # native pytorch ddp
        ...
        if int(os.environ.get("RANK", 0)) == 0:
            ...
            estimate_zero2_model_states_mem_needs_all_live(model, num_gpus_per_node=local_world_size, num_nodes=world_size // local_world_size)
    return model
...
def init_optimizer_and_scheduler(args, configs, model, gan):
    if gan is False:
        ...
        if args.train_engine == "deepspeed":
            ...
            model, optimizer, _, scheduler = deepspeed.initialize(args=args, model=model, optimizer=None, lr_scheduler=scheduler, model_parameters=model.parameters())
...
def save_model(model, model_name, info_dict):
    ...
    if info_dict["train_engine"] == "torch_ddp":
        ...
        with torch.no_grad():
            model.save_checkpoint(save_dir=model_dir, tag=model_name, client_state=info_dict)
    if rank == 0:
...
def log_per_step(writer, info_dict):
    ...
    if writer is not None:
        if (info_dict["train_engine"] == "deepspeed" and info_dict["is_gradient_accumulation_boundary"] is True) or (info_dict["train_engine"] == "torch_ddp" and (info_dict["batch_idx"] + 1) % info_dict["accum_grad"] == 0):
    ...
    if (info_dict["batch_idx"] + 1) % info_dict["log_interval"] == 0:
        ...
        if tag == "TRAIN":
            log_str += "lr {:.8f} grad_norm {:.6f}".format(info_dict["lr"], info_dict["grad_norm"])
...
def log_per_save(writer, info_dict):
    ...
    lr = info_dict["lr"]
    rank = int(os.environ.get("RANK", 0))
    logging.info("Epoch {} Step {} CV info lr {} {} rank {}".format(epoch, step + 1, lr, rank, " ".join(["{}_{}".format(k, v) for k, v in loss_dict.items()])))
