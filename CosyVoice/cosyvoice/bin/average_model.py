...
def main():
    ...
    if args.val_best:
        ...
        yamls = [f for f in yamls if not (os.path.basename(f).startswith("train") or os.path.basename(f).startswith("init"))]
        for y in yamls:
            with open(y, "r") as f:
        ...
        path_list = [args.src_path + "/epoch_{}_whole.pt".format(score[0]) for score in sorted_val_scores[: args.num]]
    print(path_list)
    avg = {}
    num = args.num
