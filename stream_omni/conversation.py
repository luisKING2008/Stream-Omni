...
class ChatItem:
    def __init__(self, msg="", text="", image=None, image_process_mode=None, audio=None):
        self.msg = msg
        self.text = text
        self.image = image
    ...
    def __repr__(self):
        return f"Chat(msg={self.msg}, text={self.text}, " f"image={self.image}, image_process_mode={self.image_process_mode}, audio={self.audio})"
...
class Conversation:
    ...
    def get_images(self, return_pil=False):
        ...
        for i, (role, msg) in enumerate(self.messages[self.offset :]):
            if i % 2 == 0:
                if type(msg) is tuple and len(msg) >= 3:
                    if len(msg) == 3:
                        msg, image, image_process_mode = msg
                        audio = None
                    else:
                        msg, image, image_process_mode, audio = msg
        ...
        for i, (role, msg) in enumerate(self.messages[self.offset :]):
            if i % 2 == 0:
                ...
                if type(msg) is tuple:
                    ...
                    ret.append([None, (audio,)])

                else:
                    if type(msg) is str:
                        msg = "`\n" + msg.strip() + "\n`"
                    ret[-1][-1] = msg
        # print("=======to_gradio_chatbot",ret)
        return ret

    def to_gradio_chatbot_from_chat(self):
        ret = []
        for i, item in enumerate(self.chatbot[self.offset :]):
            # print(item)
            if i % 2 == 0:
                msg, text, image, image_process_mode, audio = item.msg, item.text, item.image, item.image_process_mode, item.audio
                if type(image) != list:
                    image = [image]
                ...
                img_str_list = []
                for img in image:
                    if img is not None and self.is_image_file(img):
                        img_b64_str = self.process_image(img, "Default", return_pil=False, image_format="JPEG")
                        img_str = f'<img src="data:image/jpeg;base64,{img_b64_str}" style="max-width: 256px; max-height: 256px; width: auto; height: auto; object-fit: contain;"/>'
                        img_str_list.append(img_str)
...
conv_llava_llama_3_1 = Conversation(
    ...
    tokenizer=AutoTokenizer.from_pretrained("ictnlp/stream-omni-8b/"),
    stop_token_ids=[128009, 128008, 128001],
    stop_str="<|eot_id|>",
)

conv_stream_omni_llama_3_1 = Conversation(
    system="You are a helpful language and vision assistant. " "You are able to understand the visual content that the user provides, and assist the user with a variety of tasks using natural language.",
    # roles=("<|start_header_id|>user", "<|start_header_id|>assistant"),
    roles=("user", "assistant"),
    version="stream_omni_llama_3_1",
    messages=[],
    offset=0,
    sep_style=SeparatorStyle.LLAMA_3_1,
    tokenizer=AutoTokenizer.from_pretrained("ictnlp/stream-omni-8b/"),
...
conv_templates = {
    ...
    "stream_omni_llama_3_1": conv_stream_omni_llama_3_1,
}
