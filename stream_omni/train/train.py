...
from stream_omni import conversation as conversation_lib
...
class DataArguments:
    ...
    data_paths: List[str] = field(default_factory=list, metadata={"help": "Path to the training data, in llava's instruction.json format. Supporting multiple json files via /path/to/{a,b,c}.json"})
...
def preprocess_multimodal(sources: Sequence[str], data_args: DataArguments) -> Dict:
    ...
    for source in sources:
        ...
        for idx, sentence in enumerate(source):
            if idx == 0 and DEFAULT_IMAGE_TOKEN not in sentence["value"]:
...
def preprocess_llama_3_1(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False):
    conv = conversation_lib.default_conversation.copy()
    roles = {"human": conv.roles[0], "gpt": conv.roles[1]}
    ...
    for conversation, target in zip(conversations, targets):
        ...
        if cur_len < tokenizer.model_max_length:
            if cur_len != total_len:
                # target[:] = IGNORE_INDEX
                print(f"WARNING: tokenization mismatch: {cur_len} vs. {total_len}." f" (ignored)")
                # print("111111",sources)
                # print("222222",has_image)
...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_paths: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None, folder=None):
        image_folder = folder
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if type(image_file) is list:
                image = [self.process_image(f, folder=image_folder) for f in image_file]
                # Handling multi images
                # overwrite to process with simple pad
                if len(image_file) > 1:
                    image = [self.process_image(f, "pad", folder=image_folder) for f in image_file]
                    image = [[im[0], im[1], "image"] for im in image]
            else:
                image = [self.process_image(image_file, folder=image_folder)]
            sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)

            # print("444444444",self.list_data_dict[i])
            # print("000000000",sources)
            ...
            try:
                ...
                if self.data_args.add_time_instruction:
                    time_instruciton = (
                        f"The video lasts for {video_time:.2f} seconds, and {num_frames_to_sample} frames are uniformly sampled from it. These frames are located at {frame_time}.Please answer the following questions related to this video."
                    )
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    train_dataset = LazySupervisedDataset(tokenizer=tokenizer, data_paths=data_args.data_paths, data_args=data_args)
