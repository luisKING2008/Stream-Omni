...
def eval_model(args):
    ...
    for input_ids, line in tqdm.tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        ans_file.write(json.dumps({"question_id": idx, "prompt": cur_prompt, "output": whisper_asr, "answer": line["answers"], "answer_id": ans_id, "model_id": model_name, "metadata": {}}, ensure_ascii=True) + "\n")
    ans_file.close()
