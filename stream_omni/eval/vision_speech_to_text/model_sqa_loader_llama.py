...
def eval_model(args):
    ...
    questions = load_from_disk(args.data)
    ...
    for input_ids, line in tqdm.tqdm(zip(data_loader, questions), total=len(questions)):
        idx = line["Wav Filename"]
        cur_prompt = line["QuestionText"]
        ...
        ans_file.write(json.dumps({"question_id": idx, "prompt": cur_prompt, "output": outputs, "speech_token": speech_token, "answer": line["Answer"], "answer_id": ans_id, "model_id": model_name, "metadata": {}}, ensure_ascii=True) + "\n")
    ans_file.close()
