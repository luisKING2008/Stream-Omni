...
import torch
from stream_omni.model import *
from stream_omni.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from stream_omni.utils import rank0_print
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", attn_implementation=None, customized_config=None, overwrite_config=None, **kwargs):
    ...
    if "llava" in model_name.lower() or "omni" in model_name.lower() or is_multimodal:
        # Load LLaVA model
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                if "audio" in model_name.lower() or "omni" in model_name.lower():
                    from stream_omni.model.language_model.stream_omni_llama import StreamOmniConfig

                    tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                    if customized_config is None:
                        llava_cfg = StreamOmniConfig.from_pretrained(model_base)
                    ...
                    # checkpoint=safetensors.torch.load_file(model_path+"/model-00001-of-00004.safetensors")
                    # model = StreamOmniLlamaForCausalLM._from_config(llava_cfg)
                    # import pdb

                    model = StreamOmniLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, ignore_mismatched_sizes=True, **kwargs)
                    ...
                    tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                    if customized_config is None:
                        llava_cfg = LlavaConfig.from_pretrained(model_base)
                        if "v1.5" in model_name.lower():
                            llava_cfg.delay_load = True  # a workaround for correctly loading v1.5 models
                    else:
                        llava_cfg = customized_config
                    ...
                    model = LlavaLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
            ...
            if "mixtral" in model_name.lower():
                ...
                if "audio" in model_name.lower() or "omni" in model_name.lower():
                    ...
                    tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
                    if customized_config is None:
                        llava_cfg = StreamOmniConfig.from_pretrained(model_path)
                        if "v1.5" in model_name.lower():
                            llava_cfg.delay_load = True  # a workaround for correctly loading v1.5 models
                    else:
                        llava_cfg = customized_config
                    ...
                    model = StreamOmniLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, ignore_mismatched_sizes=True, **kwargs)
